  ...
  inputSchema: {
    ...
    properties: {
      ...
      showInferred: {
        type: "boolean",
        description:
          "Show inferred schemas alongside declared schemas (default: true)",
        default: true,
      },
      pageSize: {
        type: "number",
...
function buildTerminalOutput(
  tables: Array<{
    ...
    declaredFields: Array<{ name: string; type: string; optional: boolean }>;
    documents: Array<Record<string, unknown>>;
  }>,
  ...
  if (!hasAdminAccess) {
    lines.push("");
    lines.push(
      "*Note: Document counts and samples require admin access. Set CONVEX_DEPLOY_KEY for full access.*",
    );
  }
  lines.push("");
  ...
  if (selectedTable) {
    ...
    if (tableInfo) {
      ...
      for (const field of tableInfo.inferredFields) {
        lines.push(
          `| ${field.name} | ${field.type} | ${field.optional ? "No" : "Yes"} |`,
        );
      }
      lines.push("");
  ...
  for (const table of tables) {
    ...
    const fields =
      table.declaredFields.length > 0
        ? table.declaredFields
        : table.inferredFields;
    const schemaType =
      table.declaredFields.length > 0 ? "Declared" : "Inferred";
    ...
    if (table.documents.length > 0) {
      lines.push(
        `**Sample (${table.documents.length} of ${table.documentCount}):**`,
      );
      lines.push("```json");
