...
export const schemaDiagramTool = {
    name: "schema_diagram",
...
export async function handleSchemaDiagram(client, args = {}) {
    const { theme = "github-dark", ascii = false, tables: filterTables, } = args;
...
2. Or set \`CONVEX_DEPLOY_KEY\` environment variable`,
    ...
    try {
        ...
        const tableSchemas = await Promise.all(tablesToInclude.map(async (tableName) => {
            const schema = await client.getTableSchema(tableName);
            const tableInfo = allTables.find((t) => t.name === tableName);
            return {
                name: tableName,
                fields: schema.declaredFields.length > 0
        ...
        let uiUrl = "";
        try {
            const themeConfig = THEMES[theme] || THEMES["github-dark"];
        ...
        lines.push(`Tables: ${tablesToInclude.length} | Relationships: ${relations.length}`);
        lines.push("");
        ...
        if (relations.length > 0) {
            ...
            for (const rel of relations) {
                lines.push(`- \`${rel.from}\` ${rel.cardinality} \`${rel.to}\` (via ${rel.field})`);
            }
            lines.push("");
        }
 ...
 */
function detectRelationships(tables, allTableNames) {
    const relations = [];
    ...
    for (const table of tables) {
        for (const field of table.fields) {
            ...
            if (match) {
                ...
                const targetTable = allTableNames.find((t) => t.toLowerCase() === possibleTable ||
            ...
            if (idMatch) {
                ...
                if (allTableNames.includes(targetTable) && targetTable !== table.name) {
                    // Avoid duplicates
                    const exists = relations.some((r) => r.from === table.name &&
                        r.to === targetTable &&
                        r.field === field.name);
                    if (!exists) {
                        relations.push({
...
function simplifyType(type) {
    if (type.includes("v.id"))
        return "id";
    if (type.includes("string"))
        return "string";
    if (type.includes("number") || type.includes("float"))
        return "number";
 ...
 */
function generateDiagramHtml(svg, mermaidCode, theme) {
